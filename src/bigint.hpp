...
namespace crypto {
    ...
    public:
        ...
        FORCE_INLINE static Word add(BigInt<N>& r, const BigInt<N>& a, const BigInt<N>& b) {
        ...
        FORCE_INLINE static Word sub(BigInt<N>& r, const BigInt<N>& a, const BigInt<N>& b) {
        ...
        FORCE_INLINE static void mul_add_carry(Word& hi, Word& lo, Word a, Word b, Word c, Word carry_in) {
             Word prod_lo, prod_hi;
             prod_lo = _umul128(a, b, &prod_hi);
        ...
        bool is_zero() const {
            Word acc = 0;
            for (size_t i = 0; i < N; ++i) acc |= limbs[i];
            return acc == 0;
        }

        bool get_bit(size_t bit) const {
            if (bit >= N * 64) return false;
